...
router.post('/', rbac(['USER', 'SUPER_ADMIN', 'ADMIN']), uploadComplaintPhoto, complaintController.create);
router.get('/', rbac(['USER', 'SUPER_ADMIN', 'ADMIN', 'TCP']), complaintController.getAll);
router.patch('/:id/review', rbac(['TCP', 'SUPER_ADMIN', 'ADMIN']), complaintController.review);
router.patch(
  '/:id/ship',
  rbac(['TCP', 'SUPER_ADMIN', 'ADMIN']),
  uploadComplaintReplacementProof,
